...
export default function EditCustomerModal({ customer, isOpen, onClose }: EditCustomerModalProps) {
  ...
  const [requirements, setRequirements] = useState(customer.requirements);
  const [status, setStatus] = useState<CustomerStatus>(customer.status);
  const [priority, setPriority] = useState<Priority>(customer.priority);
  const [tags, setTags] = useState<string[]>(customer.tags);
  const [valueTier, setValueTier] = useState<ValueTier>(customer.valueTier);
  const [directImport, setDirectImport] = useState<DirectImport>(customer.directImport);
  
  // Dates
  const [lastFollowUpDate, setLastFollowUpDate] = useState(customer.lastFollowUpDate);
  const [nextFollowUpDate, setNextFollowUpDate] = useState(customer.nextFollowUpDate);
  
  // Notes
  const [lastContactNotes, setLastContactNotes] = useState(customer.lastContactNotes);
  const [keyMeetingPoints, setKeyMeetingPoints] = useState(customer.keyMeetingPoints);
  
  // Reset form when customer changes
  useEffect(() => {
    setIsReturningCustomer(customer.isReturningCustomer);
    ...
    setRequirements(customer.requirements);
    setStatus(customer.status);
    setPriority(customer.priority);
    setTags(customer.tags);
    setValueTier(customer.valueTier);
    setDirectImport(customer.directImport);
    setLastFollowUpDate(customer.lastFollowUpDate);
    setNextFollowUpDate(customer.nextFollowUpDate);
    setLastContactNotes(customer.lastContactNotes);
    setKeyMeetingPoints(customer.keyMeetingPoints);
  }, [customer]);
